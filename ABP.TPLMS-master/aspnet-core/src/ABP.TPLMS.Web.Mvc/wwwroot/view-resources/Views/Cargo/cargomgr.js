...
//刷新数据
function initable() {
    $("#dgCargo").datagrid({//在table中创建数据网格
        url: "/Cargo/List",//控制器中的list方法
        title: "货物管理",
        pagination: true,
        pageSize: 10,
        pageList: [10, 20, 30],
        ...
        if (rows.length > 0) {
            $.messager.confirm("提示", "确定要删除吗?", function (res) {
                if (res) {
                    ...
                    $.post("/Cargo/Delete", { "ids": codes.join(',') }, function (data) {//使用控制器中的delete方法进行删除
                        if (data == "OK") {
...
function saveDetail() {
    $.messager.confirm('确认', '您确认要修改吗？', function (r) {
        ...
        $.post("/Cargo/Update", postData, function (data) {//更新数据使用控制器中的方法，不是直接在JS中实现的，常规MVC方式是在控制器中实现需要的方法
            // alert(data); 
            var obj = JSON.parse(data);
            if (obj.result == "OK") {
